...
const CATEGORY_GROUPS = [
  {
    ...
    label: "Weltweit",
    icon: "🌍",
    description: "Alle Flughäfen im Datensatz",
    direct: true,
    filter: () => [...AIRPORTS],
  },
  {
    ...
    items: [
      {
        ...
        description: "Die 100 meistfrequentierten Flughäfen",
        filter: () => AIRPORTS.slice(0, 100),
      },
      {
        id: "large", label: "Große Flughäfen", icon: "✈️",
        description: "Internationale Großflughäfen",
        filter: () => AIRPORTS.filter(ap => AIRPORT_SIZE.large.has(ap[0])),
      },
      {
        id: "medium", label: "Mittlere Flughäfen", icon: "🛫",
        description: "Mittelgroße internationale Airports",
        filter: () => AIRPORTS.filter(ap => !AIRPORT_SIZE.large.has(ap[0]) && !AIRPORT_SIZE.small.has(ap[0])),
      },
      {
        id: "small", label: "Regionalflughäfen", icon: "🛩️",
        description: "Kleinere regionale Flughäfen",
  ...
  },
  {
    ...
    icon: "🌎",
    items: [
      { id: "europe",        label: "Europa",       icon: "🌍", description: "Europäische Flughäfen",              filter: () => AIRPORTS.filter(ap => CONTINENT_MAP[ap[3]] === "europe") },
      { id: "north_america", label: "Nordamerika",  icon: "🌎", description: "USA, Kanada, Mexiko & Karibik",      filter: () => AIRPORTS.filter(ap => CONTINENT_MAP[ap[3]] === "north_america") },
      { id: "south_america", label: "Südamerika",   icon: "🌎", description: "Südamerikanische Flughäfen",         filter: () => AIRPORTS.filter(ap => CONTINENT_MAP[ap[3]] === "south_america") },
      { id: "africa",        label: "Afrika",        icon: "🌍", description: "Afrikanische Flughäfen",             filter: () => AIRPORTS.filter(ap => CONTINENT_MAP[ap[3]] === "africa") },
      { id: "asia",          label: "Asien",         icon: "🌏", description: "Asien inkl. Naher Osten",           filter: () => AIRPORTS.filter(ap => CONTINENT_MAP[ap[3]] === "asia") },
      { id: "oceania",       label: "Ozeanien",      icon: "🌏", description: "Australien & Pazifik",              filter: () => AIRPORTS.filter(ap => CONTINENT_MAP[ap[3]] === "oceania") },
    ],
  },
  {
    id: "country",
    ...
    icon: "🏳️",
    items: [
      { id: "usa",       label: "USA",                   icon: "🇺🇸", filter: () => AIRPORTS.filter(ap => ap[3] === "USA") },
      { id: "canada",    label: "Kanada",                icon: "🇨🇦", filter: () => AIRPORTS.filter(ap => ap[3] === "Canada") },
      { id: "germany",   label: "Deutschland",           icon: "🇩🇪", filter: () => AIRPORTS.filter(ap => ap[3] === "Germany") },
      { id: "france",    label: "Frankreich",            icon: "🇫🇷", filter: () => AIRPORTS.filter(ap => ap[3] === "France") },
      { id: "uk",        label: "Vereinigtes Königreich",icon: "🇬🇧", filter: () => AIRPORTS.filter(ap => ap[3] === "UK") },
      { id: "spain",     label: "Spanien",               icon: "🇪🇸", filter: () => AIRPORTS.filter(ap => ap[3] === "Spain") },
      { id: "italy",     label: "Italien",               icon: "🇮🇹", filter: () => AIRPORTS.filter(ap => ap[3] === "Italy") },
      { id: "brazil",    label: "Brasilien",             icon: "🇧🇷", filter: () => AIRPORTS.filter(ap => ap[3] === "Brazil") },
      { id: "japan",     label: "Japan",                 icon: "🇯🇵", filter: () => AIRPORTS.filter(ap => ap[3] === "Japan") },
      { id: "china",     label: "China",                 icon: "🇨🇳", filter: () => AIRPORTS.filter(ap => ap[3] === "China") },
      { id: "australia", label: "Australien",            icon: "🇦🇺", filter: () => AIRPORTS.filter(ap => ap[3] === "Australia") },
      { id: "india",     label: "Indien",                icon: "🇮🇳", filter: () => AIRPORTS.filter(ap => ap[3] === "India") },
    ],
  },
  {
    id: "special",
    ...
    icon: "⭐",
    items: [
      { id: "capitals",  label: "Hauptstädte",    icon: "🌃", description: "Airports in Hauptstädten",     filter: () => AIRPORTS.filter(ap => CAPITAL_AIRPORTS.has(ap[0])) },
      { id: "islands",   label: "Insel-Airports", icon: "🏝️", description: "Flughäfen auf Inseln",         filter: () => AIRPORTS.filter(ap => ISLAND_AIRPORTS.has(ap[0])) },
      { id: "mountains", label: "Gebirgs-Airports",icon: "🏔️", description: "Hochgelegene Airports >1.000m",filter: () => AIRPORTS.filter(ap => HIGH_ALTITUDE_AIRPORTS.has(ap[0])) },
      { id: "coastal",   label: "Küsten-Airports", icon: "🌊", description: "Direkt am Meer gelegen",       filter: () => AIRPORTS.filter(ap => COASTAL_AIRPORTS.has(ap[0])) },
      { id: "extreme",   label: "Extreme Airports",icon: "❄️", description: "Spektakuläre Besonderheiten",  filter: () => AIRPORTS.filter(ap => EXTREME_AIRPORTS.has(ap[0])) },
    ],
  },
];
